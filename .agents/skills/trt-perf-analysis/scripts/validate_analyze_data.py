...
"""Validate analyzer JSON structure against schemas/analyze-data.schema.json.
...
Semantic pass/fail status remains in the document's validation fields.
"""
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate TensorRT analysis JSON schema and optionally require fully passed semantics."
    )
    ...
    parser.add_argument(
        "--require-passed",
        action="store_true",
        help="Also require a fully passed layer/profile analysis after schema validation.",
    )
...
def semantic_errors(data: Any) -> List[str]:
    ...
    if validation.get("status") != "passed":
        errors.append(f"$.validation.status: expected 'passed', got {validation.get('status')!r}")

    for index, result in enumerate(data.get("results", [])):
        result_validation = result.get("validation", {})
        if result_validation.get("status") != "passed":
            errors.append(
                f"$.results[{index}].validation.status: expected 'passed', got {result_validation.get('status')!r}"
            )
        if result_validation.get("analysis_mode") != "layer_profile":
            errors.append(
                "$.results["
                f"{index}].validation.analysis_mode: expected 'layer_profile', "
                f"got {result_validation.get('analysis_mode')!r}"
            )
    return errors
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if errors:
        sys.stderr.write(f"{args.input}: SCHEMA FAIL ({len(errors)} error(s))\n")
    ...
    sys.stdout.write(f"{args.input}: SCHEMA PASS\n")
    if args.require_passed:
        errors = semantic_errors(data)
        if errors:
            sys.stderr.write(f"{args.input}: SEMANTIC FAIL ({len(errors)} error(s))\n")
            for error in errors[: args.max_errors]:
                sys.stderr.write(f"- {error}\n")
            remaining = len(errors) - args.max_errors
            if remaining > 0:
                sys.stderr.write(f"... {remaining} more error(s) omitted\n")
            return 1
        sys.stdout.write(f"{args.input}: SEMANTIC PASS\n")
    return 0
