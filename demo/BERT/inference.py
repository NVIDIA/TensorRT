...
if __name__ == '__main__':
    ...
    with open(args.engine, 'rb') as f, trt.Runtime(TRT_LOGGER) as runtime, \
        ...
        for idx in range(engine.num_optimization_profiles):
            ...
            if profile_shape[0][0] <= args.batch_size and profile_shape[2][0] >= args.batch_size:
        ...
        def inference(features, tokens):
            ...
            for feature_index, feature in enumerate(features):
                # Copy inputs
                input_ids_batch = np.vstack([feature.input_ids] * args.batch_size)
                segment_ids_batch = np.vstack([feature.segment_ids] * args.batch_size)
                input_mask_batch = np.vstack([feature.input_mask] * args.batch_size)
