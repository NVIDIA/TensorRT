...
class KontextPipeline(DiffusionPipeline):
    """
    Application showcasing the acceleration of Kontext pipelines using Nvidia TensorRT.
    """

    def __init__(
        self,
        version="kontext.1-dev",
        ...
        **kwargs,
        ...
        """
        Initializes the Kontext pipeline.

        Args:
            version (`str`, defaults to `kontext.1-dev`)
                Version of the underlying Kontext model.
    ...
    @classmethod
    def FromArgs(cls, args: argparse.Namespace, pipeline_type: PIPELINE_TYPE) -> KontextPipeline:
        """Factory method to construct a `KontextPipeline` object from parsed arguments.
    ...
    def download_onnx_models(self, model_name: str, model_config: dict[str, Any]) -> None:
        ...
        if not os.path.exists(model_path):
            ...
            #Because Kontext repo does not have VAE and VAE_encoder, we use the Flux.1-dev-onnx repo
            if model_name in ["vae", "vae_encoder"]:
                snapshot_download(
                    repo_id="black-forest-labs/FLUX.1-Depth-dev-onnx",
    ...
    def is_native_export_supported(self, model_config: dict[str, Any]) -> bool:
        if self.version.startswith("kontext.1") and model_config["use_fp4"]:
    ...
    def _initialize_models(self, framework_model_dir, int8, fp8, fp4):
        ...
        if "transformer" in self.stages:
            self.models["transformer"] = KontextTransformerModel(
                **models_args,
                bf16=self.bf16,
                fp16=self.fp16,
                int8=int8,
            ...
            # #Handle the case transformer dynamic shape
            # self.models["transformer"].min_image_shape = 960
            # self.models["transformer"].max_image_shape = 1800
    ...
    def initialize_latents(
        ...
        return latents, latent_image_ids, image_latents, image_ids
    ...
    def denoise_latent(
        ...
        with torch.autocast("cuda", enabled=do_autocast):
            ...
            for step_index, timestep in enumerate(timesteps):
                # Prepare latents
                latents_input = latents if image_latents is None else torch.cat((latents, image_latents), dim=1)
                # prepare inputs
    ...
    def infer(
        ...
        with torch.inference_mode(), trt.Runtime(TRT_LOGGER):
            ...
            if input_image:
                ...
                latent_kwargs.update({"image_latents": image_latents})

            # Initialize latents
            latents, latent_image_ids, image_latents, image_ids = self.initialize_latents(
    ...
    def run(
        ...
        **kwargs,
        ...
        for _ in range(batch_count):
            print("[I] Running Kontext pipeline")
