...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Options for Kontext Demo", conflict_handler="resolve"
    )
    parser = dd_argparse.add_arguments(parser)
    parser.add_argument(
        "--version",
        type=str,
        default="kontext.1-dev",
        choices=("kontext.1-dev"),
        help="Version of Kontext",
    )
    parser.add_argument(
        "--prompt2",
        default=None,
    ...
    parser.add_argument(
        "--denoising-steps", type=int, default=28, help="Number of denoising steps"
    )
    parser.add_argument(
        "--guidance-scale",
        type=float,
...
if __name__ == "__main__":
    print("[I] Initializing Kontext demo using TensorRT")
    args = parse_args()
    ...
    # Initialize demo
    demo = pipeline_module.KontextPipeline.FromArgs(args, pipeline_type=pipeline_module.PIPELINE_TYPE.IMG2IMG)
