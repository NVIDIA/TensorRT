...
from polygraphy.backend.trt import (TacticRecorder, TacticReplayer,
                                    create_network)
...
class TestTrtConfigArgs(object):
    ...
    @pytest.mark.parametrize("arg, flag", [
        ("--int8", "INT8"),
        ("--fp16", "FP16"),
        ("--tf32", "TF32"),
    ])
    ...
    @pytest.mark.skipif(mod.version(trt.__version__) < mod.version("8.0"), reason="Bugged before TRT 8")
    @pytest.mark.parametrize("opt, cls", [
    ...
    def test_legacy_calibrator_params(self, trt_config_args):
        ...
        trt_config_args.parse_args(["--int8", "--calibration-base-class=IInt8LegacyCalibrator",
                                    "--quantile", str(quantile), "--regression-cutoff", str(regression_cutoff)])
    ...
    def test_no_deps_profiles_int8(self):
        ...
        arg_group.parse_args(["--trt-min-shapes=input:[1,25,25]", "--trt-opt-shapes=input:[2,25,25]", "--trt-max-shapes=input:[4,25,25]", "--int8"])
