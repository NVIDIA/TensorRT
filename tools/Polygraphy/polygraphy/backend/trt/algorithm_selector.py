...
class Algorithm(object):
    ...
    def from_trt(context, algorithm):
        ...
        outputs = tuple(unpack_io_info(algorithm.get_algorithm_io_info(i)) for i in range(context.num_inputs, context.num_inputs + context.num_outputs))
...
def decode(dct):
    ...
    return Algorithm(implementation=dct["implementation"],
                     tactic=dct["tactic"],
    ...
    class TacticReplayerClass(get_base_selector_type()):
        ...
        def select_algorithms(self, context, choices):
            ...
            if context.name not in self.data:
                G_LOGGER.warning("Layer: {:} was not found in the tactic replay. Falling back to default tactics. "
                                "Has the network changed since the tactic replay file was generated? "
                                "Note: Layers in the tactic replay are: {:}".format(context.name, list(self.data.keys())))
                return default_choices
            ...
            if to_select not in tactic_choices:
                G_LOGGER.critical("Tactic was not provided by TensorRT as a choice for this layer. Has the "
                                "network or builder configuration changed since the replay file was generated? "
                                "Note:\nTactic was: {:}\nAvailable choices were: {:}".format(to_select, list(map(str, tactic_choices))))
            ...
            for (context, choice) in zip(contexts, choices):
                if context.name in self.data:
                    ...
                    if to_select != selected:
                        G_LOGGER.critical("TensorRT selected a tactic different than the one specified in the tactic replay. "
                                            "Note: Tactic in replay file was: {:}, but TensorRT selected: {:}".format(to_select, selected))
