    ...
    def run(runners, data_loader=None, warm_up=None,
        ...
        def execute_runner(runner, loader_cache):
            with runner as active_runner:
                ...
                G_LOGGER.info("Runner: {:40} | Input Metadata: {:}".format(active_runner.name, input_metadata), mode=LogMode.ONCE)
                ...
                if warm_up:
                    G_LOGGER.start("Runner: {:40} | Running {:} warm-up runs".format(active_runner.name, warm_up))
                    try:
                ...
                for index, feed_dict in enumerate(loader_cache):
                    ...
                    G_LOGGER.info("Runner: {:40} | Output Metadata: {:}".format(active_runner.name, output_metadata), mode=LogMode.ONCE)
                ...
                G_LOGGER.finish("Runner: {:40} | Completed {:} iterations.".format(active_runner.name, index + 1))
        ...
        for runner in runners:
            G_LOGGER.start("Runner: {:40} | Activating and starting inference".format(runner.name))
            if use_subprocess:
                runner_queue = Queue()
                ...
                while process.is_alive() and iteration_results is None:
                    try:
                        iteration_results = misc.try_receive_on_queue(runner_queue, timeout=subprocess_polling_interval / 2)
            ...
            for result in results:
                for output_name, output in result.items():
                    ...
                    with G_LOGGER.indent():
                        ...
                        if output_valid:
                            G_LOGGER.finish("Runner: {:40} | Output: {:} is valid".format(runner_name, output_name))
                        else:
