...
class ArtifactSorterArgs(BaseArgs):
    def __init__(self, iter_art_default=None, prefer_artifacts=True, enable_iter_art=True):
        assert iter_art_default or not enable_iter_art, "Must provide iter_art_default if intermediate artifact is enabled"
    ...
    def add_to_parser(self, parser):
        ...
        artifact_sorter_args.add_argument("--artifacts", help="Path(s) of artifacts to sort. "
                                          ...
                                          nargs="+")
        artifact_sorter_args.add_argument("--art-dir", "--artifacts-dir", metavar="DIR", dest="artifacts_dir",
                                          help="The directory in which to move artifacts and sort them into 'good' and 'bad'. ")

        artifact_sorter_args.add_argument("--check", "--check-inference", dest="check", help="A command to check the model. "
                                          "The command should return an exit status of 0 for the run to be considered 'good'. "
                                          "Non-zero exit statuses are treated as 'bad' runs.", required=True, nargs=argparse.REMAINDER)

        artifact_sorter_args.add_argument("--fail-code", "--fail-returncode", dest="fail_codes",
                                          help="The return code(s) from the --check command to count as failures. "
                                          "If this is provided, any other return code will be counted as a success. Useful for ignoring certain "
                                          "types of failures. ", nargs="+", default=None, type=int)
        artifact_sorter_args.add_argument("--fail-regex", dest="fail_regex",
                                          ...
                                          "When combined with --fail-code, only iterations which return one of the specified codes are "
                                          "checked for regular expressions indicating failure.",
                                          default=None, nargs="+")

        artifact_sorter_args.add_argument("--show-output", help="Show output from the --check command. Defaults to capturing output instead. ",
                                          action="store_true")

        if self._enable_iter_art:
            artifact_sorter_args.add_argument("--iter-artifact", "--intermediate-artifact", dest="iter_artifact",
            ...
            artifact_sorter_args.add_argument("--no-remove-intermediate", help="Do not remove the intermediate artifact between iterations. "
                                              "This allows you to exit the tool early and still have access to the intermediate artifact. ",
                                              action="store_false", dest="remove_intermediate")

        artifact_sorter_args.add_argument("--iter-info", "--iteration-info", help="Path to write a JSON file containing information about "
                                          "the current iteration. This will include an 'iteration' key specifying the current iteration. ",
                                          dest="iteration_info", default=None)
    ...
    def parse(self, args):
        ...
        if self.iter_artifact and os.path.exists(self.iter_artifact):
            G_LOGGER.exit("{:} already exists, refusing to overwrite.\n"
        ...
        if self.artifacts and not self.output:
            G_LOGGER.exit("An output directory must be specified if artifacts are enabled! "
                          "Note: Artifacts specified were: {:}".format(self.artifacts))

        if not self.artifacts and self._prefer_artifacts:
            G_LOGGER.warning("`--artifacts` was not specified; No artifacts will be stored during this run! "
    ...
    def sort_artifacts(self, iteration, suffix=None):
        ...
        def move_artifacts(subdir, returncode):
            ...
            for art in self.artifacts:
                ...
                if suffix:
                    basename += suffix
                name = "{:}_{:}_{:}_N{:}_ret{:}{:}".format(basename, self.start_date, self.start_time, iteration, returncode, ext)
                dest = os.path.join(self.output, subdir, name)

                if not os.path.exists(art):
                    G_LOGGER.error("Artifact: {:} does not exist, skipping.\n"
                                   "Was the artifact supposed to be generated?".format(art))
                    continue

                if os.path.exists(dest):
                    G_LOGGER.error("Destination path: {:} already exists.\n"
        ...
        with contextlib.ExitStack() as stack, G_LOGGER.indent():
            ...
            if self.show_output:
                ...
                G_LOGGER.log("========== CAPTURED STDERR ==========\n{:}".format(status.stderr.decode()), severity=stderr_log_level)
