...
class BaseMarker(object):
    ...
    def success_message(self):
        ...
        G_LOGGER.finish("To achieve acceptable accuracy, try running the {:} {:} "
                         "layer(s) in higher precision".format(which_layers, self.num_layers))


class BisectMarker(BaseMarker):
    def __init__(self, max_layers, direction) -> None:
        super().__init__(max_layers, direction, max_layers)
        self.good = self.max_layers + 1 # Pretend marking all the layers gives us good accuracy.
        self.bad = 0
...
class Precision(BaseCheckerSubtool):
    ...
    """
    def __init__(self):
        super().__init__("precision", force_strict_types=True, prefer_artifacts=False)


    def add_parser_args(self, parser):
        parser.add_argument("--mode", help="How layers are selected to run in higher precision. "
        ...
        parser.add_argument("--dir", "--direction", help="Order in which layers are marked to run in higher precision. "
                            "'forward' will start marking layers from network inputs, and 'reverse' will start "
                            "from the network outputs", choices=["forward", "reverse"], default="reverse", dest="direction")
    ...
    def setup(self, args, network):
        ...
        if self.precision == trt.float16 and not self.arg_groups[TrtConfigArgs].fp16:
            G_LOGGER.exit("Cannot mark layers to run in fp16 if it is not enabled in the builder configuration.\n"
        ...
        if self.precision == trt.float16 and not self.arg_groups[TrtConfigArgs].int8:
            G_LOGGER.warning("Using fp16 as the higher precision, but fp16 is also the lowest precision available. "
                             "Did you mean to set --int8 as well?")

        if not any([self.arg_groups[TrtConfigArgs].tf32, self.arg_groups[TrtConfigArgs].fp16, self.arg_groups[TrtConfigArgs].int8]):
            G_LOGGER.exit("Please enable at least one precision besides fp32 (e.g. --int8, --fp16, --tf32)")

        if self.arg_groups[ModelArgs].model_type == "engine":
            G_LOGGER.exit("The precision tool cannot work with engines, as they cannot be modified. "
    ...
    def mark_layers(self, network, indices):
        ...
        for index in indices:
            ...
            G_LOGGER.verbose("Running layer in higher precision: {:}".format(trt_util.str_from_layer(layer, index)))
            layer.precision = self.precision
        G_LOGGER.verbose("Marking layer(s): {:} to run in {:} precision".format(indices, self.precision))
