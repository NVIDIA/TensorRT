...
from polygraphy.tools.args import (DataLoaderArgs, ModelArgs, OnnxLoaderArgs,
                                   ...
                                   TrtConfigArgs, TrtEngineLoaderArgs,
                                   TrtEngineSaveArgs, TrtNetworkLoaderArgs,
                                   TrtPluginLoaderArgs)
...
class Convert(Tool):
    ...
    def add_parser_args(self, parser):
        ...
        onnx_args.add_argument("--fp-to-fp16", help="Convert all floating point tensors in an ONNX model to 16-bit precision. "
    ...
    def run(self, args):
        if not args.convert_to:
            ...
            if ext not in ModelArgs.EXT_MODEL_TYPE_MAPPING:
                G_LOGGER.exit("Could not automatically determine model type based on output path: {:}\n"
        ...
        if convert_type.is_onnx():
            ...
            G_LOGGER.exit("Cannot convert to model type: {:}".format(convert_type))
