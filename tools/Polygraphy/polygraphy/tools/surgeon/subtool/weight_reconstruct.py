    ...
    def run_impl_surgeon(self, args):
        def reconstruct_weights(model):
            ...
            for initializer in model.graph.initializer:
                ...
                weight_dtype = onnx.helper.tensor_dtype_to_np_dtype(
                    initializer.data_type
                )
                weight_shape = tuple(initializer.dims)
                proxy_weight_tensor = np.random.randn(*weight_shape).astype(
                    weight_dtype
                )
                if initializer.data_type == onnx.TensorProto.BFLOAT16:
                    # numpy cannot tobytes() an ml_dtypes (bfloat16) array directly, so reinterpret
                    # its 2-byte values as uint16 to get the raw bytes for the initializer.
                    proxy_weight_bytes = proxy_weight_tensor.view(np.uint16).tobytes()
                else:
                ...
                G_LOGGER.verbose(
                    f"Reconstructing weights for the {initializer.name} initializer"
                )
                num_reconstructed += 1
