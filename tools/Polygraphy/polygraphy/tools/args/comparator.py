...
from polygraphy.tools.script import (inline, make_invocable,
...
class ComparatorRunArgs(BaseArgs):
    ...
    def add_to_parser(self, parser):
        ...
        if self._write:
            comparator_args.add_argument("--save-inputs", "--save-input-data", help="[EXPERIMENTAL] Path to save inference inputs. "
                                         "The inputs (List[Dict[str, numpy.ndarray]]) will be encoded as JSON and saved",
                                         default=None, dest="save_inputs")
            comparator_args.add_argument("--save-outputs", "--save-results", help="Path to save results from runners. "
                                         "The results (RunResults) will be encoded as JSON and saved", default=None, dest="save_results")
    ...
    def add_to_script(self, script):
        ...
        comparator_run = make_invocable("Comparator.run", script.get_runners(), warm_up=self.warm_up,
...
class ComparatorCompareArgs(BaseArgs):
    ...
    def add_to_parser(self, parser):
        ...
        comparator_args.add_argument("--rtol", "--rel-tol", dest="rtol", help="Relative tolerance for output comparison. "
                                     ...
                                     nargs="+", default=None)
        comparator_args.add_argument("--atol", "--abs-tol", dest="atol", help="Absolute tolerance for output comparison. "
                                     ...
                                     nargs="+", default=None)
        comparator_args.add_argument("--validate", help="Check outputs for NaNs and Infs", action="store_true", default=None)
        ...
        comparator_args.add_argument("--top-k", help="[EXPERIMENTAL] Apply Top-K (i.e. find indices of K largest values) to the outputs before comparing them."
                                     ...
                                     "--top-k out:5",
                                     nargs="+", default=None)
        comparator_args.add_argument("--check-error-stat", help="The error statistic to check. "
        ...
        if self._load:
            comparator_args.add_argument("--load-outputs", "--load-results", help="Path(s) to load results from runners prior to comparing. "
                                         "Each file should be a JSON-ified RunResults", nargs="+", default=[], dest="load_results")
    ...
    def add_to_script(self, script, results_name):
        ...
        if self.load_results:
            ...
            script.append_suffix(safe("\n# Load results\nfor load_output in {:}:\n\t{results}.extend(RunResults.load(load_output))",
                                                   self.load_results, results=results_name))
        ...
        if len(self.runners) > 1 or self.load_results: # Only do comparisons if there's actually something to compare.
            ...
            compare_func_str = make_invocable_if_nondefault("CompareFunc.basic_compare_func", rtol=self.rtol, atol=self.atol,
                                                           check_shapes=False if self.no_shape_check else None,
                                                           fail_fast=self.fail_fast, check_error_stat=self.check_error_stat)
            compare_func = None
            if compare_func_str:
            ...
            compare_accuracy = make_invocable("Comparator.compare_accuracy", results_name, compare_func=compare_func,
                                             fail_fast=self.fail_fast)
            script.append_suffix(safe("{success} &= bool({:})\n", compare_accuracy, success=SUCCESS_VAR_NAME))
        if self.validate:
            script.append_suffix(safe("# Validation\n{success} &= Comparator.validate({results}, check_inf=True, check_nan=True)\n",
                                                   success=SUCCESS_VAR_NAME, results=results_name))
