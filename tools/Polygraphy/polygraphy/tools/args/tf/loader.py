...
class TfLoaderArgs(BaseArgs):
    ...
    def add_to_parser(self, parser):
        ...
        if self._enable_tftrt:
            ...
            tftrt_args.add_argument("--tftrt", "--use-tftrt", help="[UNTESTED] Enable TF-TRT integration", action="store_true", default=None, dest="tftrt")
    ...
    def register(self, maker):
        ...
        from polygraphy.tools.args.trt import (TrtConfigArgs,
                                               TrtEngineSaveArgs)
    ...
    def add_to_script(self, script, disable_custom_outputs=None, suffix=None):
        ...
        if model_type == "ckpt":
            ...
            G_LOGGER.exit("Model type: {:} cannot be imported with TensorFlow.".format(model_type))
        ...
        if self.freeze_graph:
            ...
            loader_name = script.add_loader(make_invocable("OptimizeGraph", loader_name), "optimize_graph", suffix=suffix)
        if self.tftrt:
            script.add_import(imports=["UseTfTrt"], frm="polygraphy.backend.tf")
            loader_str = make_invocable("UseTfTrt", loader_name, max_workspace_size=self.trt_config_args.workspace, fp16=self.trt_config_args.fp16, int8=self.trt_config_args.int8,
        ...
        WRITE_TF = "SaveGraph"
        write_tf_str = make_invocable(WRITE_TF, loader_name, path=self.save_pb, tensorboard_dir=self.save_tensorboard, engine_dir=engine_dir)
