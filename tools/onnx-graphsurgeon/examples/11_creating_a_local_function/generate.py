...
def constant_tensor_ref(self, ref_name, dtype):
    ...
    out = self.layer(op="Constant", outputs=["constant_out"], attrs={"value": attr_ref})[0]
    out.dtype = dtype
    return out
...
def constant_float_ref(self, ref_name):
    ...
    out = self.layer(op="Constant", outputs=["constant_out"], attrs={"value_float": attr_ref})[0]
    out.dtype = np.float32
    return out

@gs.Graph.register()
def transpose(self, t, perm=[]):
    out = self.layer(op="Transpose", inputs=[t], outputs=["transpose_out"], attrs={"perm": perm})[0]
    out.dtype = t.dtype
    return out
...
attn_input_embeds = gs.Variable("input", dtype=np.float32, shape=("batch", "seqlen", "emb_dim"))
attn_attrs = {
    "Wq": None,
    "Wk": None,
    "Wv": None,
...
attn_Q = attn.matmul(attn_input_embeds, attn.constant_tensor_ref("Wq", dtype=np.float32))
attn_K = attn.matmul(attn_input_embeds, attn.constant_tensor_ref("Wk", dtype=np.float32))
attn_V = attn.matmul(attn_input_embeds, attn.constant_tensor_ref("Wv", dtype=np.float32))
attn_sqrt_emb_dim = attn.constant_float_ref("sqrt_emb_dim")
attn_perm = gs.Node.AttributeRef("transpose_perm", List[int])
attn_matrix = attn.div(attn.matmul(attn_Q, attn.transpose(attn_K, perm=attn_perm)), attn_sqrt_emb_dim)
...
input_embeds = gs.Variable("input_embeds", dtype=np.float32, shape=("batch", "seqlen", emb_dim))
