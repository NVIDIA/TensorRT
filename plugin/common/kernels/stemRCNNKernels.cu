...
template <typename DType>
__global__ void resetSMemValue_kernel(void* outPtr, int samples, float val)
{
...
template <>
__global__ void resetSMemValue_kernel<half>(void* outPtr, int samples, float val)
{
...
template <int Threads>
cudaError_t KeepTopKGather(cudaStream_t stream, int N, nvinfer1::DataType dtype, int samples,
    ...
    const void* inStemsDelta, void* outDetections)
{
    ...
    if (NClass > 1)
    { // multiple classes
        status = argMaxGroup<32>(stream, N, dtype, samples, NClass, inScores, inDelta, inStemsDelta, inCountValid,
    ...
    { // Only one class
        ...
        case nvinfer1::DataType::kFLOAT:
        {
            resetSMemValue_kernel<float><<<blocks, threads, 0, stream>>>(argMaxLabelPtr, N * samples, 0);
            break;
        }
        case nvinfer1::DataType::kHALF: { break;
        }
